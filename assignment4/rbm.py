...
class RestrictedBoltzmannMachine():
    ...
    def cd1(self, visible_trainset, n_iterations=10):
        ...
        # segment samples into mini batch size
        ...
        for it in range(n_iterations):
            ...
            for i in idx:
                ...
                # print(i)
                v_0 = visible_trainset[int(i):int(i)+self.batch_size]
                ph0, h_0 = self.get_h_given_v(v_0, last=False)
                pv0, v_1 = self.get_v_given_h(h_0)
                ...
                if i % self.rf["period"] == 0 and self.is_bottom:
                    viz_rf(weights=self.weight_vh[:, self.rf["ids"]].reshape((self.image_size[0], self.image_size[1],-1)),
                           it=it, grid=self.rf["grid"], i=i//self.batch_size)

                # print progress

                if i % self.print_period == 0:
                    print("Epoch={} Minibatch number ={} recon_loss={}".format(it, int(i//self.batch_size), np.mean(np.abs(v_0 - v_1))))
    ...
    def update_params(self, v_0, h_0, v_k, h_k):
        ...
        self.delta_weight_vh = self.learning_rate * (
                    ...
                    # v_0[:, np.newaxis] @ h_0[np.newaxis] - v_k[:, np.newaxis] @ h_k[np.newaxis])
    ...
    def get_h_given_v(self, visible_minibatch, last):
        ...
        p = sigmoid(self.bias_h + np.dot(visible_minibatch, self.weight_vh))
        h = sample_binary(p)
        if last:
            h = p
    ...
    def get_v_given_h(self, hidden_minibatch):
        ...
        if self.is_top:
            ...
            # SL TODO: Please check this if something is wrong
            p_label = softmax(label_data)
            v_label = sample_categorical(p_label)

            p = np.concatenate((p_rbm, p_label), axis=1)
            v = np.concatenate((v_rbm, v_label), axis=1)


        else:

            # [TODO TASK 4.1] compute probabilities and activations (samples from probabilities) of visible layer (replace the pass and zeros below)
            p = sigmoid(self.bias_v + np.dot(hidden_minibatch, self.weight_vh.T))
            v = sample_binary(p)
    ...
    def get_h_given_v_dir(self, visible_minibatch):
        ...
        p = sigmoid(self.bias_h + np.dot(visible_minibatch, self.weight_v_to_h))
        h = sample_binary(p)
        return p, h
    ...
    def get_v_given_h_dir(self, hidden_minibatch):
        ...
        if self.is_top:
            ...
            p = sigmoid(self.bias_v + np.dot(hidden_minibatch, self.weight_h_to_v))
            v = sample_binary(p)
