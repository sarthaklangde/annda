...
class DeepBeliefNet():    
    ...
    def recognize(self,true_img,true_lbl):
        ...
        second_p, second_h = self.rbm_stack["vis--hid"].get_h_given_v_dir(vis)
        third_p, third_h = self.rbm_stack["hid--pen"].get_h_given_v_dir(second_p)
        final_input = np.concatenate((third_p, lbl), axis=1)
    ...
    def train_greedylayerwise(self, vis_trainset, lbl_trainset, n_iterations):
        ...
        try :
            ...
            # SL: We will have to take the representation from lower layer as visible_trainset for upper layer
            second_p, second_h = self.rbm_stack["vis--hid"].get_h_given_v(vis_trainset, last=False)
            self.rbm_stack["hid--pen"].cd1(visible_trainset=second_p, n_iterations=n_iterations)
            ...
            third_p, third_h = self.rbm_stack["hid--pen"].get_h_given_v(second_p, last=False)
            final_input = np.concatenate((third_p, lbl_trainset), axis=1)
            print("Shape", final_input.shape)
            self.rbm_stack["pen+lbl--top"].cd1(visible_trainset=final_input, n_iterations=n_iterations)
